...
# Сколько вакансий отправлять за обычный запуск. Остальные остаются в очереди
# и уйдут в следующий раз — в порядке queue_position. 25 сообщений ещё можно
# прочитать за раз. Снимает этот потолок только флаг --catch-up (см. batch_limit).
MAX_PER_RUN = 25

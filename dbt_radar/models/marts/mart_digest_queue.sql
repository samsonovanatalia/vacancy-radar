...
where not coalesce(pages.is_dead, false)
  ...
  -- Балл выше нуля. Ноль и минус набирают вакансии, где минусы перевесили:
  -- senior (−2) при слабой роли и без плюсов за географию. (Штрафа −5 за
  -- неподходящую локацию с 2026-09-23 нет: её исключают правила.) Пока витрина была топ-10, до них не доходил
  -- limit; очередь отправляет всё, что в неё попало, поэтому порог — явный.
  -- «> 0», а не «>= 0»: при нуле плюсы за роль и географию целиком съедены
  -- минусами. Из mart_vacancies_scored такие вакансии не пропадают — там
  -- видно, у кого какой балл.
  and vacancies.relevance_score > 0
